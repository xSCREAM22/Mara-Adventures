...
class Battle::Scene::Animation::PokeballPlayerSendOut < Battle::Scene::Animation
  def initialize(sprites, viewport, idxTrainer, battler, startBattle, idxOrder=0)
    ...
    sprites["shadow_#{battler.index}"].visible = false # Arreglo de sombra v1.3
    @sprites        = sprites
    @viewport       = viewport
  ...
  def createFollowerProcesses
    ...
    if @shadowVisible
      ...
      shadow.moveDelta(delay, 12, Graphics.width / 2, 0) # Desplazamiento hacia la posición correcta
      shadow.setColor(delay, Color.new(0, 0, 0, 0)) # Color inicial de la sombra
    end
  end
end
